...
class PostgresMassiveProductSeeder extends Seeder
{
    ...
    /**
     * Track featured products per category to ensure minimum distribution
     * IMPORTANT: Uses per-category position for 10% featured rate, not global product ID
    ...
    protected function loadLookups(): void
    {
        $categoryRows = DB::table('categories')
            ->select('id', 'parent_id', 'slug', 'has_children', 'title')
            ->where('status', 'active')
            ->orderBy('id')
            ->get()
    ...
    protected function seedProducts(): void
    {
        ...
        while ($remaining > 0) {
            ...
            for ($i = 0; $i < $currentBatchSize; $i++) {
                ...
                $isFeatured = $this->shouldBeFeatured($catId, $productId);
            ...
            // Log category stats every 5,000 products
            if ($this->productsSeeded % 5000 === 0 && $this->productsSeeded > 0 && $this->emitConsoleProgress) {
        ...
        foreach ($this->categoryProductCount as $catId => $productCount) {
            $featuredCount = $this->categoryFeaturedCount[$catId] ?? 0;
            $categoryName = $this->categoryNames[$catId] ?? "Category #{$catId}";
            $percentage = $productCount > 0 ? ($featuredCount / $productCount * 100) : 0;
    ...
    /**
     * Determine if a product should be featured based on category needs.
     * Ensures each category gets at least minFeaturedPerCategory featured products,
     * then applies 10% featured rate based on per-category position (not global product ID).
     */
    protected function shouldBeFeatured(int $catId, int $productId): bool
    {
        ...
        $this->categoryProductCount[$catId]++;
        $categoryPosition = $this->categoryProductCount[$catId];
        ...
        // After minimum is met, use 10% featured rate based on category position
        // Every 10th product in THIS category should be featured
        $isFeatured = ($categoryPosition % 10 === 0);
        if ($isFeatured) {
            $this->categoryFeaturedCount[$catId]++;
        }
    ...
    protected function logCategoryStats(): void
    {
        $this->writeOutput('');
        $this->writeOutput('  ┌─────────────────────────────────────────────────────────────────┐');
        $this->writeOutput('  │  📊 CATEGORY DISTRIBUTION (Current Progress)                    │');
        $this->writeOutput('  └─────────────────────────────────────────────────────────────────┘');
