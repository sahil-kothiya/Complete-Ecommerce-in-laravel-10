...
class ProductImageSeeder extends Seeder
{
     ...
     */
    protected int $startProductId = 1;   // static product_id start
    protected int $totalProducts  = 10000000; // how many products to seed
    protected bool $truncateTable = true;
    ...
    public function run(): void
    {
        ...
        for ($productId = $this->startProductId; $productId < $this->startProductId + $this->totalProducts; $productId += $batchSize) {
            $batch = [];
            for ($i = 0; $i < $batchSize && ($productId + $i) < ($this->startProductId + $this->totalProducts); $i++) {
                $currentProductId = $productId + $i;
                $randomImages = collect($this->images)->random(3)->values();
                foreach ($randomImages as $index => $image) {
                    $batch[] = [
                        'product_id' => $currentProductId,
                        'image_path' => 'storage/photos/1/Products/' . $image,
            ...
            $this->command->info("Inserted {$totalInserted} images so far...");
        }

        $endTime = microtime(true);
        $this->command->info("🎉 Inserted {$totalInserted} images for {$this->totalProducts} products in " . round($endTime - $startTime, 2) . "s");
    ...
    protected function prepareDatabase(): void
    {
        ...
        if ($this->truncateTable) {
            ...
            $this->command->info("🗑️ Truncated product_images table");
        }
    }
