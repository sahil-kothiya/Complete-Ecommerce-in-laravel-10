...
{
    /**
     * Men images from BulkProductsTableSeederNew1.php
     */
    protected $menImages = [
    ...
    /**
     * Women images from BulkProductsTableSeederNew1.php
     */
    ...
    {
        $this->command->info("🗑️ Truncating product_images and products tables...");
        DB::table('product_images')->truncate();
        DB::table('products')->truncate();
        $this->command->info("✅ Tables truncated successfully");

        $faker = Faker::create();
        $menImages = collect($this->menImages);
        $womenImages = collect($this->womenImages);
        $now = Carbon::now();

        $lastId = DB::table('products')->max('id') ?? 0;
        $startId = $lastId + 1;

        $totalProducts = 10000; // Adjustable: 10,000 products
        $batchSize = 1000; // Adjustable: Batch size for inserts
        $totalBatches = (int) ceil($totalProducts / $batchSize);

        // Fetch categories and their hierarchy
        $categories = Category::all()->keyBy('id');
        $topLevelCategories = $categories->whereNull('parent_id')->pluck('id')->toArray();
        $childCategories = $categories->whereNotNull('parent_id')->pluck('id', 'parent_id')->toArray();

        // Fetch brands
        $brands = Brand::all()->keyBy('id');

        // Fetch brand-category mappings
        $brandCategoryMappings = DB::table('brand_category')->get()->groupBy('brand_id')->map(function ($mappings) {
            return $mappings->pluck('category_id')->toArray();
        })->toArray();
        ...
        for ($batch = 0; $batch < $totalBatches; $batch++) {
            DB::transaction(function () use (
                ...
                for ($i = 0; $i < $batchSize; $i++) {
                    ...
                    // Randomly select a top-level category
                    $catId = $faker->randomElement($topLevelCategories);
                    $category = $categories[$catId];

                    // Determine if we assign a child category
                    $childCatId = null;
                    if (isset($childCategories[$catId])) {
                        $childCatId = $childCategories[$catId];
                        // If the child has its own children, pick one randomly
                        if (isset($childCategories[$childCatId])) {
                            $childCatId = $faker->randomElement($categories->where('parent_id', $childCatId)->pluck('id')->toArray());
                        }
                    }

                    // Select a brand that matches the selected category (or child category)
                    $validBrandIds = [];
                    foreach ($brandCategoryMappings as $brandId => $mappedCategoryIds) {
                        if (in_array($childCatId ?? $catId, $mappedCategoryIds)) {
                            $validBrandIds[] = $brandId;
                        }
                    }
                    $brandId = !empty($validBrandIds) ? $faker->randomElement($validBrandIds) : $faker->randomElement(array_keys($brandCategoryMappings));

                    // Generate product details
                    $productName = $this->generateProductName($faker, $category->title, $childCatId ? $categories[$childCatId]->title : null);
                    $slug = Str::slug($productName) . '-' . $productIndex;
                    $size = in_array($category->title, ['Fashion', 'Men', 'Women', 'Kids', 'Shoes']) ? $this->getRandomSize() : null;
                    ...
                    $productData = [
                        'id' => $startId + $i,
                        'category' => (object) ['code' => $category->code ?? $category->title, 'title' => $category->title],
                        'brand' => (object) ['code' => $brands[$brandId]->code ?? $brands[$brandId]->title, 'title' => $brands[$brandId]->title],
                        'size' => $size,
                    ];
                    $sku = $this->generateUniqueSKU($productData, $generatedSkus);
                    ...
                    $products[] = [
                        ...
                        'sku' => $sku,
                        'slug' => $slug,
                        'summary' => $this->generateProductSummary($faker, $category->title),
                        'description' => $this->generateProductDescription($faker, $category->title),
                ...
                foreach ($insertedIds as $index => $productId) {
                    $productIndex = $batch * $batchSize + $index;

                    // Alternate between men and women images for all categories
                    $imageCollection = ($productIndex % 2 === 0) ? $womenImages : $menImages;
                    $imageIndex = ($productIndex % 2 === 0) ? $womenImageIndex : $menImageIndex;

                    $selectedImages = $this->getSequentialImages($imageCollection, $imageIndex, random_int(1, 3));
                    if ($productIndex % 2 === 0) {
                        $womenImageIndex = ($womenImageIndex + count($selectedImages)) % $womenImages->count();
                    } else {
                        $menImageIndex = ($menImageIndex + count($selectedImages)) % $menImages->count();
                    }

                    foreach ($selectedImages as $imgIndex => $imageName) {
                        $productImages[] = [
        ...
        $this->command->info("🎉 Done seeding $totalProducts products with images and SKUs.");
    }

    /**
     * Generate product name based on category.
     */
    private function generateProductName($faker, $category, $childCategory = null)
    {
        $adjectives = ['Stylish', 'Premium', 'Classic', 'Modern', 'Elegant', 'Durable', 'High-Quality', 'Trendy', 'Luxury', 'Smart'];
        $productsByCategory = [
            'Electronics' => ['Smartphone', 'Laptop', 'Headphones', 'Smartwatch', 'Tablet', 'Speaker', 'Camera'],
            'Fashion' => ['T-Shirt', 'Jeans', 'Dress', 'Shoes', 'Jacket', 'Sweater', 'Blazer'],
            'Home & Kitchen' => ['Sofa', 'Table', 'Blender', 'Microwave', 'Cookware Set', 'Lamp', 'Bed'],
            'Sports & Fitness' => ['Treadmill', 'Dumbbells', 'Yoga Mat', 'Running Shoes', 'Bicycle', 'Tent'],
            'Beauty & Personal Care' => ['Moisturizer', 'Lipstick', 'Perfume', 'Shampoo', 'Face Mask', 'Mascara'],
            'Mobiles & Accessories' => ['Smartphone', 'Phone Case', 'Charger', 'Earphones'],
            'Smartphones' => ['Smartphone', 'Phone Case'],
            'Laptops & Accessories' => ['Laptop', 'Laptop Bag', 'Mouse', 'Keyboard'],
            'Audio' => ['Headphones', 'Speaker', 'Earbuds'],
            'Men' => ['Shirt', 'Pants', 'Shoes', 'Jacket', 'Tie'],
            'Shoes' => ['Sneakers', 'Boots', 'Loafers'],
            'Women' => ['Dress', 'Blouse', 'Heels', 'Handbag'],
            'Kids' => ['T-Shirt', 'Shorts', 'Sneakers', 'Backpack'],
            'Furniture' => ['Sofa', 'Chair', 'Table', 'Bed'],
            'Kitchen Appliances' => ['Blender', 'Microwave', 'Toaster', 'Coffee Maker'],
            'Gym Equipment' => ['Treadmill', 'Dumbbells', 'Resistance Bands'],
            'Outdoor' => ['Tent', 'Backpack', 'Hiking Boots'],
            'Skin Care' => ['Moisturizer', 'Face Wash', 'Sunscreen'],
            'Makeup' => ['Lipstick', 'Foundation', 'Mascara']
        ];

        $categoryKey = $childCategory ?? $category;
        $productList = $productsByCategory[$categoryKey] ?? $productsByCategory['Fashion'];
        $adjective = $faker->randomElement($adjectives);
        $product = $faker->randomElement($productList);

        return $adjective . ' ' . $product;
    }

    /**
     * Generate product summary based on category.
     */
    private function generateProductSummary($faker, $category)
    {
        $summariesByCategory = [
            'Electronics' => [
                'Cutting-edge technology for modern needs.',
                'High-performance device with sleek design.',
                'Reliable and durable electronics for daily use.'
            ],
            'Fashion' => [
                'Stylish and comfortable for everyday wear.',
                'Perfect for both casual and formal occasions.',
                'Trendy design to elevate your wardrobe.'
            ],
            'Home & Kitchen' => [
                'Enhance your home with premium quality.',
            ...
            'Sports & Fitness' => [
                ...
                'High-quality gear for fitness enthusiasts.'
            ],
            'Beauty & Personal Care' => [
                'Enhance your beauty with premium products.',
                'Gentle and effective for daily care.',
                'Luxurious care for radiant results.'
            ]
        ];

        $summaries = $summariesByCategory[$category] ?? $summariesByCategory['Fashion'];
        return $faker->randomElement($summaries);
    }

    /**
     * Generate product description based on category.
     */
    private function generateProductDescription($faker, $category)
    {
        $baseDescription = $faker->paragraph(3);
        $categorySpecific = match ($category) {
            'Electronics' => 'Engineered with the latest technology for superior performance.',
            'Fashion' => 'Designed for style and comfort, perfect for any occasion.',
            'Home & Kitchen' => 'Crafted to enhance your living space with functionality and style.',
            'Sports & Fitness' => 'Built to support your active lifestyle with durability and quality.',
            'Beauty & Personal Care' => 'Formulated to enhance your natural beauty with premium ingredients.',
            default => 'A versatile product designed for quality and performance.'
        };

        return $baseDescription . ' ' . $categorySpecific;
    }

    /**
     * Get random size for fashion products.
     */
    ...
    /**
     * Get sequential images for a product.
     */
    private function getSequentialImages($imageCollection, $startIndex, $count)
    {
        $images = [];
        $totalImages = $imageCollection->count();

        for ($i = 0; $i < $count; $i++) {
            $index = ($startIndex + $i) % $totalImages;
            $images[] = $imageCollection->get($index);
        }
     ...
     */
    private function generateUniqueSKU($productData, &$generatedSkus, $maxRetries = 100): ?string
    {
        for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
            $sku = $this->generateSKU($productData, $attempt);

            if (!isset($generatedSkus[$sku]) && !DB::table('products')->where('sku', $sku)->exists()) {
                return $sku;
            }
        }
    ...
    /**
     * Generate SKU based on product data and attempt number.
     */
    private function generateSKU($productData, int $attempt): string
    {
        $cat = $this->getCode($productData['category']->code ?? $productData['category']->title ?? 'GEN');
        $brand = $this->getCode($productData['brand']->code ?? $productData['brand']->title ?? 'GEN');
        $variant = $this->mapSize($productData['size'] ?? null) . $this->hashDigit($productData['id']);
        $unique = $this->generateUniqueID($productData['id'], $attempt);
    ...
    /**
     * Get a 3-character code from a string.
     */
    ...
    /**
     * Map size to a single character.
     */
    ...
    /**
     * Generate a single hexadecimal digit from input.
     */
    ...
    /**
     * Calculate CRC16 checksum for SKU.
     */
