...
class OptimizedMasterSeederNew extends Seeder
{
    ...
    protected function seedProductsBulk()
    {
        ...
        foreach ($categories as $categoryKey => $categoryData) {
                    ...
                    DB::transaction(function () use (
                        ...
                        for ($i = 0; $i < $actualBatchSize; $i++) {
                            ...
                            $productsData[] = [
                                ...
                                'cat_id' => $categoryData['top_id'],
                                'child_cat_id' => $categoryData['id'],
            ...
            $actualProductsInCategory = DB::table('products')
                ->where('child_cat_id', $categoryData['id'])
                ->count();
    ...
    protected function getLeafCategories()
    {
        ...
        foreach ($leafSlugs as $slug) {
            ...
            if (!$id) continue;
            $topId = $this->getTopParent($id, $allCategories);
    ...
    protected function getTopParent($catId, $categories)
    {
    ...
    {
        return collect($this->images)->random(3)->values()->toArray();
    }
    ...
    public function validateSeeding()
    {
        ...
        foreach ($categories as $category) {
            $actualCount = DB::table('products')->where('child_cat_id', $category->id)->count();
