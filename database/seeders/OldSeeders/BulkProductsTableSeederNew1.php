...
class BulkProductsTableSeederNew1 extends Seeder
{
    /**
    ...
    {
        // Truncate tables at the beginning
        $this->command->info("🗑️ Truncating product_images table...");
        DB::table('product_images')->truncate();

        $this->command->info("🗑️ Truncating products table...");
        ...
        // Image sequence trackers to avoid repetition
        $menImageIndex = 0;
        $womenImageIndex = 0;
        ...
        for ($batch = 0; $batch < $totalBatches; $batch++) {
            DB::transaction(function () use (
                ...
                for ($i = 0; $i < $batchSize; $i++) {
                    ...
                    // Alternate between men (1) and women (2): woman-man-woman-man
                    // Start with women first, then alternate
                    $catId = ($productIndex % 2 === 0) ? 2 : 1;
                    $gender = ($catId === 1) ? 'men' : 'women';

                    // Generate proper product names based on gender
                    $productName = $this->generateProductName($faker, $gender);
                    ...
                    $products[] = [
                        'title'         => $productName,
                        'slug'          => $slug,
                        'summary'       => $this->generateProductSummary($faker, $gender),
                        'description'   => $this->generateProductDescription($faker, $gender),
                ...
                foreach ($insertedIds as $index => $productId) {
                    ...
                    if ($catId === 1) { // Men products
                        $selectedImages = $this->getSequentialImages($menImages, $menImageIndex, 3);
                        $menImageIndex += 3;
                        if ($menImageIndex >= $menImages->count()) {
                            $menImageIndex = 0;
                        }
                    } else { // Women products
                        $selectedImages = $this->getSequentialImages($womenImages, $womenImageIndex, 3);
                        $womenImageIndex += 3;
                        if ($womenImageIndex >= $womenImages->count()) {
                            $womenImageIndex = 0;
                        }
                    }
    ...
    private function generateProductName($faker, $gender)
    {
        $adjectives = ['Stylish', 'Classic', 'Modern', 'Elegant', 'Comfortable', 'Premium', 'Luxury', 'Casual', 'Formal', 'Trendy'];
        $womenProducts = ['Dress', 'Blouse', 'Skirt', 'Heels', 'Handbag', 'Sandals', 'Jacket', 'Sweater', 'Jeans', 'Top'];
        $menProducts = ['Shirt', 'Pants', 'Shoes', 'Jacket', 'Sweater', 'Jeans', 'Suit', 'Blazer', 'Shorts', 'Polo'];

        $adjective = $faker->randomElement($adjectives);
        $product = $gender === 'men' ? $faker->randomElement($menProducts) : $faker->randomElement($womenProducts);

        return $adjective . ' ' . $product;
    }

    private function generateProductSummary($faker, $gender)
    {
        $womenSummaries = [
            'Elegant and comfortable for everyday wear.',
            'Perfect for both casual and formal occasions.',
            'Made with high-quality materials for lasting comfort.',
            'Stylish design that complements any wardrobe.',
            'Versatile piece suitable for various occasions.'
        ];

        $menSummaries = [
            'Classic design with modern comfort.',
            'Professional look for business and casual wear.',
            'Durable construction with premium materials.',
            'Timeless style that never goes out of fashion.',
            'Comfortable fit for all-day wear.'
        ];

        return $gender === 'women' ? $faker->randomElement($womenSummaries) : $faker->randomElement($menSummaries);
    }

    private function generateProductDescription($faker, $gender)
    {
        $baseDescription = $faker->paragraph(3);
        $genderSpecific = $gender === 'women'
            ? 'Designed specifically for women who appreciate style and comfort.'
            : 'Crafted for the modern man who values quality and style.';

        return $baseDescription . ' ' . $genderSpecific;
    }
