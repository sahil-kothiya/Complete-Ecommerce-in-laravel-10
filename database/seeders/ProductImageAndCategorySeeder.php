...
class ProductImageAndCategorySeeder extends Seeder
{
    protected $menImages = [
    ...
    {
        if (count($this->menImages) < 3 || count($this->womenImages) < 3) {
            $this->command->error("❌ At least 3 images required for both men and women.");
            return;
        }

        // Disable foreign key constraints and truncate product_images
        ...
        foreach ($products as $index => $product) {
            // Assign category IDs for first 120 products (63 women + 57 men)
            $catId = $index < 63 ? 2 : 1; // First 63 are women (cat_id 2), next 57 are men (cat_id 1)

            // Update cat_id in products table
            DB::table('products')
                ->where('id', $product->id)
                ->update(['cat_id' => $catId, 'updated_at' => $now]);

            // Select images and path based on category
            $images = ($catId === 2 && $index < 63) ? $this->womenImages : $this->menImages;
            $imagePath = ($catId === 2) ? 'storage/photos/1/Products/Woman/' : 'storage/photos/1/Products/Man/';
            $imageIndex = ($catId === 2 && $index < 63) ? $womenImageIndex : $menImageIndex;

            // For products after first 120, randomly assign cat_id and images
            if ($index >= 120) {
                $catId = rand(1, 2);
                $images = $catId === 1 ? $this->menImages : $this->womenImages;
                $imagePath = ($catId === 2) ? 'storage/photos/1/Products/Woman/' : 'storage/photos/1/Products/Man/';
                $imageIndex = array_rand($images);
            }

            // Ensure we have enough images
            $img1 = $images[$imageIndex % count($images)];
            $img2 = $images[($imageIndex + 1) % count($images)];
            $img3 = $images[($imageIndex + 2) % count($images)];

            // Increment image index for sequential assignment
            if ($catId === 2 && $index < 63) {
                $womenImageIndex += 3;
            } elseif ($catId === 1 && $index < 120) {
                $menImageIndex += 3;
            }
        ...
        $this->command->info("🎉 Done: Seeded {$productCounter} products with images and updated categories.");
    }
}
