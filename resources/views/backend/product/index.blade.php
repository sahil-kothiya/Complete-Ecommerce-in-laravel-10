...
@section('main-content')
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('product.create') }}" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="bottom"
                title="Add Product" tabindex="1">
...
$primaryImage =
    $product->has_variants && $product->variants->count() > 0
...
if ($product->has_variants) {
    $activeInStockVariants = $product->variants
        ->where('status', 'active')
        ->where('stock', '>', 0);
    ...
    $discountedPrice =
        $product->base_price * (1 - ($product->base_discount ?? 0) / 100);
    ...
    $displaySize = 'N/A';
                                ...
                                <tr>
                                    <td>{{ $serialNumber }}</td>
                                    <td tabindex="{{ $loop->iteration + 1 }}">{{ $product->title }}
                                        {{ $product->has_variants ? '(Variants)' : '' }}</td>
                                    <td>
                                    ...
                                    <td>
                                        @if ($primaryImage)
                                            <img src="{{ $primaryImage->url }}" class="img-fluid zoom"
                                    ...
                                    <td class="d-flex">
                                        <a href="{{ route('product.edit', $product->id) }}"
                                            ...
                                            title="Edit" tabindex="{{ $loop->iteration + 200 }}">
                                        ...
                                        <form method="POST" action="{{ route('product.destroy', $product->id) }}">
                                            ...
                                            <button class="btn btn-danger btn-sm dltBtn" data-id="{{ $product->id }}"
                                                style="height:30px; width:30px; border-radius:50%" data-toggle="tooltip"
                                                title="Delete" tabindex="{{ $loop->iteration + 300 }}">
