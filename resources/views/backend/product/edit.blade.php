...
<div class="card">
	...
	<div class="card-body">
		<form method="post" action="{{route('product.update',$product->id)}}">
			...
			<div class="form-group">
				<label for="inputPhoto" class="col-form-label">Upload New Image(s) <span class="text-danger">*</span></label>
				<div class="input-group mb-2">
					...
					<input id="thumbnail" class="form-control" type="text" name="photo"
						@if(!$product->images->count()) required @endif
					value="{{$product->images->pluck('image_path')->implode(',')}}">
				</div>
				@if($product->images->count())
				<small class="form-text text-muted">
					You already have {{ $product->images->count() }} uploaded image{{ $product->images->count() > 1 ? 's' : '' }}. Uploading new image(s) will replace them.
				</small>
				@else
