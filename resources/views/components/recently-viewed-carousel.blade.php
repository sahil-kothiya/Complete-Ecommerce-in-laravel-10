    ...
    <div class="container">
        <div class="d-flex align-items-center justify-content-between mb-2 flex-wrap">
            ...
            <div class="carousel-nav-btns">
                <x-carousel-nav-button :id="$prevButtonId" :aria-label="'Previous ' . strtolower($title)" icon="ti-angle-left" :direction="-1"
                    :background-color="$defaultBackgroundColor ?? '#6c757d'" :text-color="$defaultTextColor ?? 'white'" :hover-scale="$defaultHoverScale" :hover-shadow="$defaultHoverShadow" :auto-scroll-speed="$defaultAutoScrollSpeed"
                    :scroll-amount="$defaultScrollAmount" :shimmer-animation="$defaultShimmer" :carousel-id="$carouselId" />

                <x-carousel-nav-button :id="$nextButtonId" :aria-label="'Next ' . strtolower($title)" icon="ti-angle-right" :direction="1"
                    :background-color="$defaultBackgroundColor ?? '#6c757d'" :text-color="$defaultTextColor ?? 'white'" :hover-scale="$defaultHoverScale" :hover-shadow="$defaultHoverShadow" :auto-scroll-speed="$defaultAutoScrollSpeed"
                    :scroll-amount="$defaultScrollAmount" :shimmer-animation="$defaultShimmer" :carousel-id="$carouselId" />
            </div>
        </div>
            ...
            <div class="carousel-track flipkart-carousel" id="{{ $carouselId }}">
                    ...
                    @foreach ($products as $product)
                        ...
                        <div class="carousel-item flipkart-card" tabindex="{{ $tabindex }}">
                            <div class="flipkart-card-img-wrap">
                                ...
                                @if ($baseDiscount > 0)
                                    <span class="flipkart-discount-badge">{{ number_format($baseDiscount, 0) }}%
                                        Off</span>
                                @endif
                                ...
                                <div class="add-to-cart mt-2 d-flex align-items-center gap-2">
                                    @if ($basePrice > 0)
                                        <a href="{{ route('add-to-cart', $product->slug) }}"
                                            ...
                                            {{ $baseStock > 0 ? 'Add to Cart' : 'Out of Stock' }}
                                        </a>
                                    @else
                ...
                @else
                    <div class="carousel-item text-center"
                        style="min-width: {{ $itemMinWidth }}; max-width: {{ $itemMaxWidth }}; opacity: 0.7;">
