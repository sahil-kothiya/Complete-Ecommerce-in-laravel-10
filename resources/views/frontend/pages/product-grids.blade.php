...
@push('scripts')
<script>
    window.maxPrice = {{ $max_price ?? 1000 }};
</script>
...
<script>
    ...
    function initializeSlider() {
        const sliders = document.querySelectorAll('.slider-wrapper[data-slider]');
            ...
            if (images.length > 1) {
                ...
                wrapper.addEventListener('mouseenter', () => {
                    ...
                    intervalId = setInterval(() => {
                        index = (index + 1) % images.length; // Cycle through all images
                        track.style.transform = `translateX(-${index * 100}%)`;
                    }, 800); // Change image every 800ms (adjust as needed)
                });
                
                wrapper.addEventListener('mouseleave', () => {
                    // Stop the cycling and return to first image
                    if (intervalId) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const observer = new MutationObserver((mutations) => {
            if (document.querySelector('.slider-wrapper[data-slider]')) {
                initializeSlider();
                observer.disconnect();
            }
        });
        observer.observe(document.getElementById('product-grids'), { childList: true, subtree: true });
    });
    ...
    $(document).ready(function() {
        ...
        // Initial attachment for pagination (will be called after first AJAX load)
        attachPaginationListeners();
        ...
        // NEW: Load initial products via AJAX after initialization
        applyFilters(1);
        ...
        console.log('Has filters:', hasUrlFilters);
    });
</script>
@endpush