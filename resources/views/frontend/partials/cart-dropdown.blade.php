...
<div class="shopping-item">
    ...
    <ul class="shopping-list">
        @forelse(Helper::getAllProductFromCart() as $index => $data)
            ...
            <li>
                ...
                <a class="cart-img" href="{{ $productUrl }}" tabindex="{{ 3 + $index * 3 }}"><img
                        src="{{ $imageUrl }}" alt="{{ $data->product->title ?? 'Product Image' }}"
                        loading="lazy"></a>
                <h4><a href="{{ $productUrl }}" target="_blank"
                        tabindex="{{ 4 + $index * 3 }}">{{ $data->product->title ?? 'Product' }}</a></h4>
    ...
    <div class="bottom">
        ...
        <a href="{{ route('checkout') }}" class="btn animate"
            tabindex="{{ 2 + Helper::getAllProductFromCart()->count() * 3 + 1 }}">Checkout</a>
    </div>
</div>
