    ...
    <div class="topbar">
        <div class="container">
            <div class="row">
                ...
                <div class="col-lg-6 col-md-12 col-12">
                    <ul class="list-main">
                        <li><i class="ti-location-pin" aria-hidden="true"></i> <a
                                href="{{ route('order.track') }}">Track Order</a></li>
                        @auth
                            <li><i class="ti-user" aria-hidden="true"></i> <a
                                    href="{{ Auth::user()->role === 'admin' ? route('admin') : route('user') }}"
                                    target="_blank" rel="noopener">Dashboard</a></li>
                            <li><i class="ti-power-off" aria-hidden="true"></i> <a
                                    href="{{ route('user.logout') }}">Logout</a></li>
                        @else
                            <li><i class="ti-power-off" aria-hidden="true"></i> <a
                                    href="{{ route('login.form') }}">Login</a> / <a
                ...
                <div class="col-lg-2 col-md-2 col-12">
                    <div class="logo">
                        <a href="{{ route('home') }}" aria-label="Go to homepage">
                            <img src="{{ $settings->logo ?? asset('images/default-logo.png') }}" alt="Company Logo"
                                loading="eager" width="120" height="40" style="max-width: 100%; height: auto;">
                        </a>
                    </div>
                ...
                <div class="col-lg-8 col-md-7 col-12">
                    <div class="search-bar-wrapper py-3 px-3 position-relative">
                        <form action="{{ route('product.search') }}" method="GET"
                            class="form-inline search-bar-form justify-content-center" autocomplete="off">
                            <div class="input-group mb-0 w-100 w-md-auto">
                                <input type="text" name="search" id="search-input" class="form-control search-input"
                                    ...
                                    aria-controls="autocomplete-list" aria-expanded="false" role="combobox"
                                    autocomplete="off">
                            </div>
                            <div id="autocomplete-dropdown"
                                class="autocomplete-dropdown position-absolute w-100 bg-white shadow-sm border rounded mt-1 d-none">
                        ...
                        @auth
                            <div class="sinlge-bar shopping">
                                <a href="{{ route('wishlist') }}" class="single-icon"
                                    aria-label="View wishlist ({{ Helper::wishlistCount() ?? 0 }} items)">
                                    <i class="fa fa-heart-o" aria-hidden="true"></i>
                                    <span class="total-count"
                                        aria-label="{{ Helper::wishlistCount() ?? 0 }} items in wishlist">{{ Helper::wishlistCount() ?? 0 }}</span>
                                </a>
                            ...
                            <div class="sinlge-bar shopping">
                                <a href="{{ route('cart') }}" class="single-icon"
                                    aria-label="View cart ({{ Helper::cartCount() ?? 0 }} items)">
                                    <i class="ti-bag" aria-hidden="true"></i>
                                    <span class="total-count"
                                        aria-label="{{ Helper::cartCount() ?? 0 }} items in cart">{{ Helper::cartCount() ?? 0 }}</span>
                                </a>
        ...
        <div class="container">
            <nav class="navbar navbar-expand-lg" aria-label="Main navigation">
                <div class="navbar-collapse">
                    <ul class="nav main-menu menu navbar-nav mk-ct" role="menubar">
                        <li class="{{ request()->is('home') ? 'active' : '' }}" role="none"><a
                                href="{{ route('home') }}" role="menuitem">Home</a></li>
                        <li class="{{ request()->is('about-us') ? 'active' : '' }}" role="none"><a
                                href="{{ route('about-us') }}" role="menuitem">About Us</a></li>
                        <li class="{{ request()->is('product-grids', 'product-lists') ? 'active' : '' }}"
                            role="none"><a href="{{ route('product-grids') }}" role="menuitem">Products</a></li>
                        @include('frontend.partials.category-menu')
                        <li class="{{ request()->is('contact') ? 'active' : '' }} cnt-us" role="none"><a
                                href="{{ route('contact') }}" role="menuitem">Contact Us</a></li>
                    </ul>
                </div>
            </nav>
        </div>
            ...
            function performAutocomplete(query) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $list.empty();
                        if (response.success && Array.isArray(response.suggestions) && response
                            ...
                            response.suggestions.forEach((item, index) => {
                                ...
                                if (price > 0) {
                                    if (discount > 0) {
                                        ...
                                        priceHTML =
                                            `<span class="price">$${discountPrice.toFixed(2)} <del>$${price.toFixed(2)}</del></span>`;
                                    } else {
                                        priceHTML =
                                ...
                                const itemTabIndex = baseTabIndex + (index *
                                2); // Increment by 2 to account for remove button
                            ...
                            $list.html(
                                '<li class="list-group-item no-results" style="min-height: 40px;">No products found</li>'
                                );
                        }
                        showDropdown();
                    },
                    error: function(xhr, status) {
                        ...
                        $list.html(
                            `<li class="list-group-item error" style="min-height: 40px;">${errorMessage}</li>`
                            );
                        showDropdown();
                    }
                });
