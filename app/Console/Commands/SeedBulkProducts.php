...
class SeedBulkProducts extends Command
{
    protected $signature = 'seed:bulk-products {--count=1000000} {--batch=1500}';
    protected $description = 'Seed bulk products into the database with images (PostgreSQL optimized)';
    ...
    public function handle(): void
    {
        ...
        $now = Carbon::now();
        $imagePool = collect($this->images);

        $total = (int) $this->option('count');
        $batchSize = (int) $this->option('batch');
        $batches = (int) ceil($total / $batchSize);
        $startId = (DB::table('products')->max('id') ?? 0) + 1;

        $this->info("Seeding $total products in $batches batches of $batchSize...");
        $startTime = microtime(true);

        // 🔒 Disable triggers for performance (PostgreSQL-specific)
        ...
        for ($batch = 0; $batch < $batches; $batch++) {
            DB::transaction(function () use (
                ...
                for ($i = 0; $i < $batchSize; $i++) {
                    ...
                    $title = "Sample Product $index";
                    $slug = Str::slug($title) . '-' . $startId;
                ...
                foreach ($insertedIds as $productId) {
                    $images = $imagePool->random(3)->values();
                    foreach ($images as $idx => $img) {
                        $productImages[] = [
                            'product_id' => $productId,
                            'image_path' => 'storage/photos/1/Products/' . $img,
                            'is_primary' => $idx === 0,
                            'sort_order' => $idx,
                        ];
                    }
            ...
            $this->info("✅ Batch " . ($batch + 1) . "/$batches complete");
        }
        ...
        $this->info("🎉 Done seeding $total products in {$duration}s.");
    }
}
