...
class HighPerformanceFilterController extends Controller
{
    ...
    private function buildBaseStatsQuery($category, array $filters)
    {
        $query = DB::table('products')
            ->select(['id', 'price', 'discount', 'stock', 'brand_id'])
            ->where('status', 'active');
    ...
    private function getOptimizedFilteredProducts($category, array $currentFilters, Request $request)
    {
        ...
        $query = DB::table('products as p')
            ->select([
                ...
                DB::raw('ARRAY_AGG(DISTINCT pi.image_path ORDER BY pi.sort_order) FILTER (WHERE pi.image_path IS NOT NULL) as images')
            ])
    ...
    private function transformProductsOptimized($products): array
    {
        return $products->map(function ($p) {
            ...
            return [
                ...
                'images' => $p->images ? json_decode($p->images, true) : ['/images/default-placeholder.jpg'],
                'rating' => [
    ...
    {
        return RedisHelper::remember("subcats_v5_{$parentId}", 3600, function () use ($parentId) {
